...
void Canvas::saveToFile(const char* filename) {
	...
	if (ls.find(".bmp") != std::string::npos) {
		stbi_write_bmp(filename, W, H, 3, &pixels[0]);
		return;
	}
	if (ls.find(".tga") != std::string::npos) {
		stbi_write_tga(filename, W, H, 3, &pixels[0]);
		return;
	}
	if (ls.find(".jpg") != std::string::npos) {
		stbi_write_jpg(filename, W, H, 3, &pixels[0], 100);
		return;
	}
	if (ls.find(".png") != std::string::npos) {
		stbi_write_png(filename, W, H, 3, &pixels[0], 0);
		return;
	}
}